...
public:
	CheckoutRecord& operator=(const CheckoutRecord& s){
		book_id = s.book_id;
		title = s.title;
		date_borrowed = s.date_borrowed;
		date_due = s.date_due;
		borrower.first = s.borrower.first;
		borrower.second = s.borrower.second;
		wait_list.clear();
		for(std::vector<std::pair<std::string, std::string>* >::const_iterator iter = s.wait_list.begin();
				iter != s.wait_list.end(); ++iter) {
			std::pair<std::string, std::string>* ppr = new std::pair<std::string, std::string>;
			ppr->first = (*iter)->first;
			ppr->second = (*iter) -> second;
			wait->list.push_back(ppr);
		}
		return *this;
	}
